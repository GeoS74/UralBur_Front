...
function getUrlSection(alias){
  ...
  return `section.html/${levelAlias}/${alias}.html`
}
...
const alias = URL.parse(window.location).searchParams.get('levelAlias')
...
Promise.all([levelsFetch, levelsFetchAll, positionsFetch])
  ...
  .then(([levelsFetchAll, levelsFetch, positionsFetch]) => {
    ...
    return [levelsFetchAll, levelsFetch, positionsFetch];
  })
  .then(responses => {
